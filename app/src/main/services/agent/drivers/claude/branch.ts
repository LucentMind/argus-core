 ...
 *  - A fork has no undo history, so files are rewound on the ORIGINAL session first, then the
 *    fork is cut. Never the other way round.
 */
export function createClaudeBranching(deps: ClaudeBranchDeps): ClaudeBranching {
  async function fileAnchor(a: BranchArgs): Promise<string | null> {
    const msgs = await deps.messages(a.cursor, { dir: a.caseDir })
    const at = msgs.findIndex((m) => m.uuid === a.anchor)
    if (at < 0) return null
    return msgs.slice(at + 1).find((m) => m.type === 'user')?.uuid ?? null
  }
